...
def get_descriptors(model, loader, device=None):
    ...
    for batch_idx, sample in tqdm.tqdm(enumerate(loader), total=len(loader), ncols=80, leave=False):
        ...
        p_features = model.encode_faces(parent_images).detach().cpu().numpy()
        c_features = model.encode_faces(children_images).detach().cpu().numpy()
        features = np.abs(p_features - c_features)
        all_features.append(features)
        labels = list(sample["kin"].numpy())
...
for pair_type in ["fs", "fd", "ms", "md"]:
    kin_face_w_loader = kin_data.get_data_loader_full(batch_size=6,
                                                      pair_type=pair_type,
